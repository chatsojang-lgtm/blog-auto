...
조사한 정보를 항목별로 정리해서 알려주세요. 확인되지 않은 정보는 "확인 불가"로 표기해주세요.
...
앞서 작성된 내용: (서론 + 본론1 이미 작성됨)

이어서 본론2를 HTML로 작성하라:
1. 검색 결과에서 확인된 객관적 데이터를 활용하라: 가격, 위치(역에서 도보 거리), 영업시간, 주차 정보 등.
2. 검색 결과에 있는 실제 고객 리뷰나 평점을 인용하여 신뢰도를 높여라. 리뷰 정보가 없으면 이 항목은 생략하라.
3. <h3> 소제목 1~2개를 사용하라.
4. <!-- IMAGE --> 주석으로 이미지 위치 1~2곳 표시하라.
5. 분량: 300~500자 내외.
6. "${input.theme}" 테마와 연관된 실용 정보를 강조하라.
...
2. 검색 결과에서 확인된 정보를 바탕으로, 방문자가 가질 법한 질문 3~4가지를 <ul><li> 리스트로 정리하라. 각 질문에 검색 결과 기반의 답변도 포함. 확인되지 않은 정보는 포함하지 마라.
